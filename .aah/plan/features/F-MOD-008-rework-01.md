...
- **Watch List: AAV, customized-Target flag, status columns.** The Watch tab's list adds the player's `aav_minor` — `WatchListEntry` (`schema/MOD-008-api-schema.yaml`, `getWatchList`) gains an `aav_minor` field, joined the same way other panels already join primary-source AAV — a flag indicating the viewing team has a customized target for that player (cross-referenced against the already-fetched `targets` state, not a new field), and the player's injury/availability status (already present per player in the fetched player list as `injury_status`).
- **Nomination Queue: opening price and availability columns.** The Queue tab's list adds an opening-price column (the draft's configured `min_bid_minor`, from `getDraftConfig` — `GET /drafts/:draftId/config`, already implemented server-side in `server/src/draft/war-room.ts` returning `auction.min_bid_minor` among other config, but not yet documented in a schema file or fetched by this screen — one value applies to every queued player, since opening price is a draft-wide auction setting, not a per-player one) and an availability column (whether the player has already been drafted, from the same `drafted` set derived from `ws.recentAwards` that other panels already use).
...
produces:
  ...
  - operation_id: getDraftConfig
    schema_file: schema/MOD-008-api-schema.yaml
    request_schema: null
    response_schema: DraftConfigResponse
    note: >
      Gap-review addition: this endpoint (GET /drafts/:draftId/config) was already
      implemented server-side (server/src/draft/war-room.ts) but never documented in
      any schema file. Documented here so the Nomination Queue's opening-price column
      (auction.min_bid_minor) has a resolvable contract.
consumes:
