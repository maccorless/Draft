...
- Adds a readiness/status messaging section (IA §0.1: "readiness/status messaging from the commissioner if provided") — surfaced from whatever commissioner-entered status text is available on the league/draft record the Lobby already receives its props from (this module does not invent a new backend field for this; if no such field exists yet in the props/API the Lobby consumes, render nothing for this section rather than inventing one — MOD-010 owns any new commissioner-authored league setup fields).
